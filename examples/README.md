...
| [ops-console](ops-console/) | The module system for real: `@Module`, `@Controller`, `@Injectable`, DI, guards and middleware, across four domain modules — login with roles, live metrics, a process table, threshold alerts with notifications, a taskbar badge and a deep link. React + Vite, the only example with a Node toolchain. | you are building anything past a single `app.py` file — multiple domains, role-based access, or a frontend framework. |
...
The vanilla examples (`hello`, `media-vault`, `launcher`) need nothing beyond that.
`ops-console` is a React project and additionally needs `npm install` first — see its
own README.
